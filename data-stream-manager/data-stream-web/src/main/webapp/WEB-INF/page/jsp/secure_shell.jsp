...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<%= request.getContextPath() %>/statics/css/termin/jquery-ui/base/jquery-ui.css"/>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(".submit_btn").button().click(function () {
                <c:if test="${pendingSystemStatus!=null}">
            ...
            $(".cancel_btn").button().click(function () {
                window.location = 'getNextPendingSystemForTerms.action?pendingSystemStatus.id=<c:out value="${pendingSystemStatus.id}"/>&script.id=<c:if test="${script!=null}"><c:out value="${script.id}"/></c:if>&_csrf=<c:out value="${sessionScope._csrf}"/>';
            });
            ...
            <c:choose>
               <c:when test="${currentSystemStatus!=null && currentSystemStatus.statusCd=='GENERICFAIL'}">
                     $("#error_dialog").modal();
               </c:when>
               <c:when test="${currentSystemStatus!=null && currentSystemStatus.statusCd=='HOSTFAIL'}">
                     $("#error_dialog").modal();
               </c:when>
               <c:when test="${pendingSystemStatus!=null}">
                  <c:choose>
                    <c:when test="${pendingSystemStatus.statusCd=='AUTHFAIL'}">
                          $("#set_password_dialog").modal();
                    </c:when>
                    <c:when test="${pendingSystemStatus.statusCd=='KEYAUTHFAIL'}">
            ...
            function resize(element) {
                ...
                if (termMap[id]) {
                    ...
                    $.ajax({
                        url: '../admin/setPtyType?id=' + id + '&ptyWidth=' + width + '&ptyHeight=' + height,
                        cache: false
                    });
                }
            }
            ...
            function  createTermMap(id, output){
                ...
                <c:if test="${userSettings !=null && userSettings.bg !=null}">
                    termMap[id].colors[256] = '<c:out value="${userSettings.bg}"/>';
                </c:if>
                <c:if test="${userSettings !=null && userSettings.fg!=null}">
                    termMap[id].colors[257] = '<c:out value="${userSettings.fg}"/>';
                </c:if>
            ...
            <c:forEach var="i" items="${systemList}">
              $(createTermElement(<c:out value="${i.instanceId}"/>,<c:out value="${i.id}"/>,'<c:out value="${i.displayLabel}"/>')).appendTo(".termwrapper");
              setTerminalEvents($("#run_cmd_"+<c:out value="${i.instanceId}"/>));
            </c:forEach>
...
<c:when test="${(systemList!= null && !systemList.isEmpty()) || pendingSystemStatus!=null}">
            ...
            <c:if test="${pendingSystemStatus==null}">
                ...
                <ul class="nav navbar-nav">
                    <li><a id="select_all" href="#"
                           title="Use CMD-Click or CTRL-Click to select multiple individual terminals">Select All</a></li>
                    ...
                    <%--<li class="dropdown">--%>
                        ...
                        <%--</ul>--%>
                    <%--</li>--%>
                    <li><a href="/view/system">Exit Terminals</a></li>
                </ul>
...
<div class="term-container container">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                            <select id="connectHostId" class="host_frm_select">
                                <c:forEach var="i" items="${allocatedSystemList}">
                                    <option value="${i.id}">${i.displayLabel}</option>
                                </c:forEach>
                            </select>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Enter password for <c:out
                            value="${pendingSystemStatus.displayLabel}"/></h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="error">Error: <c:out value="${pendingSystemStatus.errorMsg}"/></div>
                        <form id="password_frm" name="password_frm" action="/admin/createTerms.action" method="post">
                            <table class="wwFormTable">
                                <input type="hidden" name="pendingSystemStatus.id" id="password_frm_pendingSystemStatus_id" value="${pendingSystemStatus.id}">
                                <tbody>
                                  <tr>
                                      <td class="tdLabel">
                                          <label for="password_frm_password" class="label">Password</label>
                                      </td>
                                      <td>
                                          <input type="password" name="password" id="password_frm_password">
                                      </td>
                                  </tr>
    ...
    <div id="set_passphrase_dialog" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close cancel_btn" data-dismiss="modal" aria-hidden="true">x</button>
                    <h4 class="modal-title">Enter passphrase for <c:out
                            value="${pendingSystemStatus.displayLabel}"/></h4>
                </div>
                <div class="modal-body">
                    <div class="row">
    ...
    <div id="error_dialog" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">x</button>
                    <h4 class="modal-title">System: <c:out value="${currentSystemStatus.displayLabel}"/></h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="error">Error: <c:out value="${currentSystemStatus.errorMsg}"/></div>
    ...
    <form id="composite_terms_frm" name="composite_terms_frm" action="/admin/createTerms.action" method="post">
         <input type="hidden" name="pendingSystemStatus.id" value="${pendingSystemStatus.id}">
         <c:if test="${script!=null}">
